...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['product_id'])) {
    ...
    try {
        ...
        if (mysqli_num_rows($product_result) === 0) {
            ...
            $check_wishlist_sql = "SELECT id FROM wishlist WHERE product_id = ? AND session_id = ?";
            $check_wishlist_stmt = mysqli_prepare($conn, $check_wishlist_sql);
            mysqli_stmt_bind_param($check_wishlist_stmt, "is", $product_id, $session_id);
            ...
            if (mysqli_num_rows($wishlist_result) > 0) {
                ...
                // Add to wishlist
                $sql = "INSERT INTO wishlist (product_id, session_id) VALUES (?, ?)";
